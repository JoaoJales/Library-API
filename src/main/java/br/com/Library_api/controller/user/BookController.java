...
public class BookController {
    ...
    @Operation(summary = "Consultar livros pelo título")
    @GetMapping("/search")
    @PreAuthorize("hasAnyRole('STUDENT', 'PROFESSOR', 'VISITOR', 'ADMIN')")
    public ResponseEntity<Page<GetDetailingBookDTO>> searchBooks( @PageableDefault(size = 10, sort = "title") Pageable pageable, @RequestParam String title) {
        Page<GetDetailingBookDTO> page = bookService.searchBooksByTitle(title, pageable);
