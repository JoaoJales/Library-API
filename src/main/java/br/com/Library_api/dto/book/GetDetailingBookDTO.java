...
public record GetDetailingBookDTO(Long id, String title, Integer publicationYear, String publisher, String genre, String isbn, GetAuthorSummaryDTO summaryAuthor, List<GetBookCopyDTO> copies) {
    public GetDetailingBookDTO(Book book){
        this(book.getId(), book.getTitle(), book.getPublicationYear(), book.getPublisher(), book.getGenre(), book.getIsbn(),
                ...
                book.getCopies().stream().map(GetBookCopyDTO::new).toList()
        );
    }
}
